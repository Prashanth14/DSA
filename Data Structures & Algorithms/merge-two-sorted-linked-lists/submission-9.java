...
class Solution {
    // Approach: dummy head + tail pointer, merge by picking smaller node each step
    // TC: O(n + m) - n, m are lengths of list1 and list2, each node visited once
    // SC: O(1) - no extra nodes created, just relinking existing nodes
